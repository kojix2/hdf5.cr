module HDF5
  class Attributes
    ...
    def []=(name : String, value : T) forall T
      ...
      {% else %}
        {% raise "Use attrs[name] = value for scalar types (Number, String). For arrays use set_array." %}
      {% end %}
    end

    def []=(name : String, value : Array(T)) forall T
      LibHDF5.H5Adelete(@loc_id, name) if has_key?(name)
      {% if T == String %}
      ...
      {% else %}
        {% raise "Unsupported array element type for attribute: #{T}" %}
      {% end %}
    end
